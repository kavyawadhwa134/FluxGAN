...
loss_log_file = './plots/loss_log_improved.csv'
...
# Improved physics constraints with better enrichment handling
physics_weight = 0.015  # Slightly reduced
temperature_tolerance = 50.0
burnup_correlation_weight = 0.01
thermal_hydraulics_weight = 0.01
fuel_performance_weight = 0.005
enrichment_weight = 0.005  # Separate weight for enrichment
...
if not os.path.exists(loss_log_file):
    with open(loss_log_file, 'w') as f:
        f.write('Epoch,D Loss,G Loss,Temp Loss,Thermal Loss,Fuel Loss,Burnup Loss,Enrichment Loss,GenMean,GenStd\n')
...
# Calculate actual enrichment bounds from data
enrichment_data = X[:, 0]
enrichment_min = np.min(enrichment_data)
enrichment_max = np.max(enrichment_data)
print(f"Actual enrichment range in data: {enrichment_min:.2f}% - {enrichment_max:.2f}%")

class ImprovedPhysicsConstraints:
    """Physics constraints with improved enrichment handling"""
    
    def __init__(self, data_min, data_max, enrichment_min, enrichment_max):
        self.data_min = torch.tensor(data_min, device=device)
        self.data_max = torch.tensor(data_max, device=device)
        self.enrichment_min = enrichment_min
        self.enrichment_max = enrichment_max
    ...
    def enrichment_specific_loss(self, generated_data, enrichment_cond):
        """Improved enrichment-specific constraints"""
        denorm_data = self.denormalize(generated_data)
        denorm_enrichment = self.denormalize(enrichment_cond)[:, 0]
        
        enrichment = denorm_data[:, 0]
        
        # 1. Bounds based on actual data range (with small tolerance)
        tolerance = 2.0  # 2% tolerance
        bounds_loss = torch.mean(torch.relu(torch.clamp(self.enrichment_min - tolerance - enrichment, max=10.0))) + \
                     torch.mean(torch.relu(torch.clamp(enrichment - (self.enrichment_max + tolerance), max=10.0)))
        
        # 2. Enrichment consistency with condition (should be close)
        consistency_loss = torch.mean(torch.abs(enrichment - denorm_enrichment))
        
        # 3. Enrichment effects on other parameters (gentler)
        fuel_temp = denorm_data[:, 3]
        clad_temp = denorm_data[:, 4]
        
        # Simple correlation checks (gentler penalties)
        try:
            enrich_temp_corr = torch.corrcoef(torch.stack([enrichment, fuel_temp]))[0, 1]
            if torch.isnan(enrich_temp_corr):
                enrich_temp_corr = torch.tensor(0.0, device=device)
            enrich_clad_corr = torch.corrcoef(torch.stack([enrichment, clad_temp]))[0, 1]
            if torch.isnan(enrich_clad_corr):
                enrich_clad_corr = torch.tensor(0.0, device=device)
            correlation_loss = torch.relu(-enrich_temp_corr * 0.1) + torch.relu(-enrich_clad_corr * 0.1)
        except:
            correlation_loss = torch.tensor(0.0, device=device)
        
        return bounds_loss + 0.1 * consistency_loss + correlation_loss
    
    def physical_bounds_loss(self, generated_data):
        """Physical bounds with improved enrichment handling"""
        denorm_data = self.denormalize(generated_data)
        
        bounds_loss = 0.0
        
        # Enrichment: Use actual data bounds with tolerance
        enrichment = denorm_data[:, 0]
        tolerance = 2.0
        bounds_loss += torch.mean(torch.relu(torch.clamp(self.enrichment_min - tolerance - enrichment, max=10.0))) + \
                      torch.mean(torch.relu(torch.clamp(enrichment - (self.enrichment_max + tolerance), max=10.0)))
...
# Initialize physics constraints with actual enrichment bounds
physics_constraints = ImprovedPhysicsConstraints(data_min, data_max, enrichment_min, enrichment_max)

# Generator with improved initialization
...
# Discriminator with improved initialization
...
# Initialize models
generator = ImprovedGenerator(noise_dim, cond_dim=1).to(device)
discriminator = ImprovedDiscriminator(cond_dim=1).to(device)

# Optimizers with improved learning rates
optimizer_G = optim.Adam(generator.parameters(), lr=0.00008, betas=(0.5, 0.999), weight_decay=1e-6)
optimizer_D = optim.Adam(discriminator.parameters(), lr=0.000015, betas=(0.5, 0.999), weight_decay=1e-6)

# Learning Rate Scheduler
scheduler_G = StepLR(optimizer_G, step_size=2000, gamma=0.9)
scheduler_D = StepLR(optimizer_D, step_size=2000, gamma=0.9)
...
def add_instance_noise(data, std=0.002):  # Reduced noise
    """Add Gaussian noise to data for instance noise regularization."""
    return data + torch.randn_like(data) * std

# Checkpoint functions
def save_checkpoint(epoch):
    path = os.path.join(checkpoint_dir, f'checkpoint_improved_{epoch}.tar')
    torch.save({
        'epoch': epoch,
...
for epoch in range(start_epoch, num_epochs):
    for real_data, cond in dataloader:
        ...
        with autocast(device_type='cuda' if torch.cuda.is_available() else 'cpu'):
            ...
            # Instance noise
            real_data_noisy = add_instance_noise(real_data, std=0.002)
            ...
            fake_data_noisy = add_instance_noise(fake_data.detach(), std=0.002)
        ...
        # ====== Train Generator with Improved Physics Loss ======
        ...
        with autocast(device_type='cuda' if torch.cuda.is_available() else 'cpu'):
            ...
            # Improved physics-informed losses
            ...
            enrichment_loss = physics_constraints.enrichment_specific_loss(fake_data, cond)
            bounds_loss = physics_constraints.physical_bounds_loss(fake_data)
            
            # Combined physics loss with separate enrichment weight
            physics_loss = (temp_loss + 
                          ...
                          enrichment_weight * enrichment_loss + 
                          bounds_loss)
    ...
    if epoch % 1 == 0:
        with torch.no_grad():
            ...
            enrichment_loss_log = physics_constraints.enrichment_specific_loss(fake_data_log, cond_log).item()
            
        print(f"Epoch [{epoch}/{num_epochs}] | D Loss: {d_loss.item():.4f} | G Loss: {g_loss.item():.4f} | Temp: {temp_loss_log:.4f} | Thermal: {thermal_loss_log:.4f} | Fuel: {fuel_loss_log:.4f} | Burnup: {burnup_loss_log:.4f} | Enrich: {enrichment_loss_log:.4f} | GenMean: {gen_mean:.3f} | GenStd: {gen_std:.3f}")
        with open(loss_log_file, 'a') as f:
            f.write(f'{epoch},{d_loss.item()},{g_loss.item()},{temp_loss_log},{thermal_loss_log},{fuel_loss_log},{burnup_loss_log},{enrichment_loss_log},{gen_mean},{gen_std}\n')
...
print("Improved FluxGAN training completed!") 